...
def generate_answer(fact_type, fact_value, direction=None, interest=None):
    templates = {
        ...
        "предпочтения": [
            ...
            f"Определенно да! На направлениях {direction} ты сможешь развивать свои интересы в {fact_value}",
    ...
    if fact_type == "предпочтения":
        ...
        # Вопрос о направлениях для конкретного интереса
            ...
            if directions:
                ...
                return random.choice([
                    f"Интерес к {interest} представлен в направлениях: {dir_list}.",
                    f"Вы можете изучать {interest} на следующих направлениях: {dir_list}.",
                    f"Направления, связанные с {interest}: {dir_list}."
                ])
            else:
